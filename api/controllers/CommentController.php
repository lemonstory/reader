        ...
        if ($ret['data']['totalCount'] > 0) {
            ...
            if ($ret['data']['currentPage'] == 1 && $ret['data']['totalCount'] >= $ret['data']['commentTotalMinCount']) {
                ...
                $commentHotIdArr = Comment::find()
                    ...
                    ->limit(Yii::$app->params['commentHotMaxCount'])
                    ->asArray()
                    ->all();
    ...
    private function processCommentHierarchy($commentPairIdArr, $commentContentArr)
    {
        ...
        if (!empty($commentPairIdArr) && !empty($commentContentArr)) {
            ...
            foreach ($commentPairIdArr as $commentId => $parentCommentId) {
                ...
                $lastModifyTimeObj = Carbon::createFromTimestamp($commentContentArr[$commentId]['last_modify_time']);
                if($lastModifyTimeObj->isCurrentYear()) {
                    $comment['last_modify_time'] = $lastModifyTimeObj->diffForHumans();
                }else {
                    $comment['last_modify_time'] = $lastModifyTimeObj->toDateTimeString();
                }
